...
def sqm_plot2020(dataframe, ars_dataframe, plot_ars=False):
    """
    ...
    # Plot 1: Time series of mags
    x = []
    y = []
    # for date, mags_list in zip(dataframe.index, dataframe['mags']):
    #     if isinstance(mags_list, list):
    #         for mags in mags_list:
    #             x.extend([date]*len(mags_list))
    #             y.extend(mags_list)
    # ax[0].scatter(x, y, s=0.3, color='black', alpha=0.3)

    if not plot_ars:
        # Plot the SQM measurements
        ax[0].scatter(dataframe.index, dataframe['mags'], s=0.3, color='black', alpha=0.3)
        ...
        ax[0].set_title('SQM Time Series 2020')
    elif plot_ars:
        # Plot the SQM measurements
        ax[0].scatter(dataframe.index, dataframe['mags'], s=0.3, color='black', alpha=0.3)
        ax[0].set_xlim(dataframe.index.min(), dataframe.index.max())
        ...
        # Plot the ARS particulate matter
        ax2 = ax[0].twinx()
        ax2.scatter(dataframe.index, ars_dataframe['AQI'],  s=0.3, marker='x', color='red', alpha=0.3)
        ax2.set_ylim(0, 150)
        ax2.set_ylabel(fr'PM2.5 [µg/m^3]')
        ax[0].set_ylim(15, 22)
        ax[0].set_xlabel('Date')
        ax[0].set_title('SQM Time Series 2020')
    ...
    # Now, each month with have 5 box plots, one for each lunar phase:
    positions = []
    ...
    ax[1].set_title(f'Monthly SQM Magnitudes by Lunar Phase 2020: {title}')
    ax[1].grid()
    ...
    ax[2].set_ylim(15, 22)
    ax[2].set_title(f'Monthly SQM Magnitudes by Cloud Cover 2020: {title}')
    ax[2].grid()

    # Plot 3: Pull from ARS 2020 data to make a box plot of mags vs ARS particular matter
    # first, bin the ARS PM2.5 AQI's
